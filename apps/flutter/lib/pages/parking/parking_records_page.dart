...
class _ParkingRecordsPageState extends State<ParkingRecordsPage> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Center(child: ConstrainedBox(constraints: BoxConstraints(maxWidth: 600), child: _loading ? const Center(child: CircularProgressIndicator()) : ListView(padding: const EdgeInsets.all(16), children: _items.map((r) => Card(child: ListTile(title: Text(r['plate_number'] ?? ''), subtitle: Text('进入: ${r['enter_time'] ?? '-'} | 离开: ${r['leave_time'] ?? '-'}')))).toList()))),
  );
}
