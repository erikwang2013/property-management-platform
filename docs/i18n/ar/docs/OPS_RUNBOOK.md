...
طرفا admin و service يتشاركان نفس مثيل MySQL وقاعدة `property_management`، نسخة واحدة تكفي. المدخل الموحد:

| اسم القاعدة | سكربت النسخ | الوصف |
|---|---|---|
| `property_management` | `scripts/backup.sh` | يقرأ الاتصال من `admin/.env` (يمكن تجاوز اسم الحاوية بـ `--container=`)، افتراضيًا mysqldump داخل الحاوية |
...
3. التحقق:
   - مقارنة الصفوف: `SELECT COUNT(*) FROM erik_user;` مطابق لما سُجل قبل النسخ
