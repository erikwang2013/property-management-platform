...
> 아래 Mermaid 다이어그램은 GitHub / GitLab / VS Code에서 자동 렌더링됩니다. 다른 환경에서는 [Mermaid Live Editor](https://mermaid.live/)를 사용하세요.
...
flowchart TB
    subgraph "客户측层"
        ...
        A2["Flutter Web<br/>PC 业主측<br/>(Port 3001)"]
        A3["HarmonyOS ArkTS<br/>手机/平板客户측"]
    end
    ...
    subgraph "应用层"
        ...
        C2["service webman :8788<br/>业主측 API<br/>JWT 认证"]
    end
...
sequenceDiagram
    participant U as 用户
    participant CL as 客户측
    participant SV as 服务측
