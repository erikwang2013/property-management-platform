...
<img src="../docs/images/pet_xiaozhu.svg" alt="小筑 · 项目宠物" width="100" align="right">
...
| 📈 报表中心 | 日期范围筛选 + 收支趋势/收缴率/业务分布/欠费排行 + PDF 导出 | Redis 缓存 5 分钟 |
...
```bash
bash ../scripts/deploy.sh   # 仓库根目录一键脚本：生成 .env + 密钥 + Docker Compose 启动 + 建库
```

详见根目录 [README.md](../README.md) 的「一键安装脚本」说明。
...
- **页面**: 登录、仪表盘、报表中心、用户管理、角色权限、系统配置、操作日志、个人中心
...
1. **登录**：默认账号 `admin / admin123`，登录后请立即修改密码。
2. **基础数据**：小区 → 楼栋 → 单元 → 户型 → 房产 → 业主绑定，逐步录入。
3. **业务运营**：费用类型 → 批量生成账单 → 线下收款；报修派单/进度；投诉处理/回访；报表中心查看收支与收缴情况。
4. **系统管理**：新增管理员、角色授权（RBAC）、系统配置、操作日志审计。
