...
class SecurityPatrolController extends BaseCrudController {
  final patrols = <Map<String, dynamic>>[].obs;
  @override String get basePath => ApiConfig.securityPatrol;
  @override List<dynamic> get items => patrols;
  @override void onLoadSuccess(Map<String, dynamic> d) {
    patrols.value = List<Map<String, dynamic>>.from(d['data'] ?? []);
...
class PatrolRecordController extends GetxController {
  ...
  Future<void> loadItems({bool reset = false}) async {
    ...
    try {
      final r = await api.get(ApiConfig.patrolRecord, params: {'page': page.value, 'limit': 15});
