...
class MultiTenantTest extends TestCase
{
    ...
    public function test_schema_has_tenant_columns_and_backfill(): void
    {
        ...
        $this->assertStringContainsString('CREATE TABLE IF NOT EXISTS `management_platform_tenant`', $sql);
        $this->assertMatchesRegularExpression('/CREATE TABLE IF NOT EXISTS `management_community`.*?`tenant_id` BIGINT UNSIGNED NOT NULL DEFAULT 0/s', $sql);
        $this->assertMatchesRegularExpression('/CREATE TABLE IF NOT EXISTS `management_admin_user`.*?`tenant_id` BIGINT UNSIGNED NOT NULL DEFAULT 0/s', $sql);
        $this->assertStringContainsString('INSERT IGNORE INTO `management_platform_tenant`', $sql);
        $this->assertStringContainsString('UPDATE `management_community` SET `tenant_id` = 1 WHERE `tenant_id` = 0', $sql);
        $this->assertStringContainsString('UPDATE `management_admin_user` SET `tenant_id` = 1 WHERE `tenant_id` = 0', $sql);
    }
