...
```
management_admin_user ──┬── management_admin_user_role ──┬── management_admin_role
  (用户)           │    (用户-角色关联)         │     (角色)
                  │                          │
...
management_system_config (系统配置) — 独立表
```
...
| 表名 | 字段数 | 说明 |
|------|-------|------|
| `management_admin_user` | 14 | 管理用户，phone/email/id_card 加密存储，支持软删除 |
| `management_admin_role` | 7 | 角色，slug 唯一 |
| `management_admin_permission` | 10 | 权限树（parent_id 自引用），type: 1=菜单 2=按钮 3=API |
| `management_admin_user_role` | 2 | 用户-角色多对多中间表 |
| `management_admin_role_permission` | 2 | 角色-权限多对多中间表 |
| `management_system_config` | 8 | 键值对配置，group+key 联合唯一 |
| `management_operation_log` | 9 | 操作审计日志（含 source 来源端） |
