...
class CleaningAreaController extends BaseController
{
    /**
     * 保洁区域列表
    ...
    public function show(Request $request, string $hashid)
    {
        ...
        $item = CleaningArea::find($id);
        if (!$item) {
            return $this->fail('保洁区域不存在', 404);
        }
    ...
    {
        $data = $request->only([
            'community_id', 'name', 'location', 'area', 'frequency',
