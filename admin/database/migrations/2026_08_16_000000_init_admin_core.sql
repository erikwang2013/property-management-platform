...
-- 迁移: 初始化管理后台核心数据表（增量拆分自 docs/install.sql）
-- 主键 id 使用 BIGINT UNSIGNED NOT NULL，由 snowflake-php 应用层生成
-- 拆分规则: 本文件为迁移路径第 1 步——管理后台核心域
--   (erik_admin_user/role/permission/user_role/role_permission/system_config/operation_log)
-- 增量升级: 按文件名序号顺序执行；全量安装仍用 docs/install.sql
-- 注意: operation_log 的 source 列 ALTER 为 install.sql 内历史演进，原样保留
-- ============================================================
-- ============================================================
-- Copyright (c) 2026 erik <erik@erik.xyz> — https://erik.xyz
-- 迁移: 初始化管理后台核心数据表
