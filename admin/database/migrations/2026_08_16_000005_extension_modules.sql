...
-- 迁移: 功能扩展域（23 张）
-- 拆分规则: 消息通知/审批/支付/投票/SLA/催缴/巡检/商城/人脸/集团/智能问答
--   11 模块 (notification_template/notification/approval_type/approval/
--   approval_record/payment_order/vote/vote_option/vote_record/sla_rule/
--   sla_record/collection_strategy/collection_record/inspection_task/
--   inspection_checkpoint/mall_category/mall_product/mall_order/face_info/
--   group/group_community/knowledge_base/chat_record)
-- 归属说明: 数据大屏/ES搜索无独立表（复用现有表统计/索引），故无迁移文件；
--   跨域引用（repair_order/bill/owner 等）均指向更早批次，无循环依赖
